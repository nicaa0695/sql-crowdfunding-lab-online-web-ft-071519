...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  ...
  AS diff FROM pledges JOIN projects ON projects.id = pledges.project_id GROUP BY pledges.project_id HAVING diff >= 0 ORDER BY projects.title"
end
...
def selects_the_category_name_and_the_sum_total_of_the_all_its_pledges_for_the_books_category
  "SELECT projects.category, users.name, SUM(pledges.amount) FROM projects JOIN pledges ON projects.id = pledges.project_id
